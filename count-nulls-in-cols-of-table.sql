...
--
with q(qry) as (
  with nullqueries(qry, tblname) as (
    select 'COUNT(*) FILTER (WHERE ' || quote_ident(attname) || ' is null) AS ' || quote_ident(attname::text) || '_numnull', quote_ident(c.relnamespace::regnamespace::text) || '.' || quote_ident(c.relname::text)
